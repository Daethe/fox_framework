...
class Model {
	...
	/**
	 * Default ID column for all models. Can be overridden by adding a public static _id_column property to your model classes.
	 */
	...
	protected static function _class_name_to_table_name($class_name) {
		return strtolower(preg_replace(['/\\\\/', '/(?<=[a-z])([A-Z])/', '/__/'], ['_', '_$1', '_'], ltrim($class_name, '\\')));
	}
	 ...
	 * @return ParisWrapper
	 */
	public static function factory($class_name, $connection_name = NULL, $class_namespace = '\App\Model\\') {
		$class_name = self::$auto_prefix_models . $class_name;
		$table_name = self::_get_table_name($class_name);

		if ($connection_name == NULL) {
			$connection_name = self::_get_static_property($class_name, '_connection_name', Idiorm::DEFAULT_CONNECTION);
		}
		$wrapper = ParisWrapper::for_table($table_name, $connection_name);
		$wrapper->set_class_name($class_name);
		$wrapper->set_class_namespace($class_namespace);
	...
	/**
	 * Calls static methods directly on the ParisWrapper
	 *
	 * @param  string $method
